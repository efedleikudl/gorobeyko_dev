...
// Official Kubernetes helm mark, traced as a single SVG path on a 24x24 viewBox.
// Particles are laid out along this outline so the constellation reads as the
// Kubernetes logo.
export const KUBERNETES_LOGO_PATH =
...
export function createParticleOptions(isNarrow: boolean) {
  return {
    ...
    polygon: {
      ...
      // The path lives on a 24-unit viewBox, so scale sets the on-screen size
      // (24 * scale px tall).
      scale: isNarrow ? 10 : 15,
